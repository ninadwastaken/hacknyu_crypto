...
def get_tax(memo, user):
    ...
    if wolfram_output:
        prompt = f"I have {user.balance}$ in my bank account. " \
                 ...
                 f"The average price of {memo.product_name} is {wolfram_output}" \
    ...
    return int(response.choices[0].message.content.strip()) * user.balance // 2000
